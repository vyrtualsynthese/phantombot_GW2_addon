 ...
 *
 *    V3.6.0
 *    - !gw2 wvw [matchup | rank | tick | score] added to get for more consistente and ability to know the matchup and servers names.
 *    - !gw2 local [en | fr | de | es | ko | zh] enable ability to chose differents localisations pour the API
 *
 *    V3.5.2
 ...
 *    - Updated language files to support both ranking systems.
 *
 *    V3.3.1
...
(function(){
    ...
    var GW2_matchup;
    var GW2_matchup_red;
    var GW2_matchup_blue;
    var GW2_matchup_green;
    var GW2_matchup_red_name = [];
    var GW2_matchup_blue_name = [];
    var GW2_matchup_green_name = [];
    var GW2_red_tick = null;
    var GW2_blue_tick = null;
    var GW2_green_tick = null;
    var GW2_wvw_vp;
    var GW2_wvw_score;
    var GW2_commander;
    var GW2_guilds;
        ...
        if (command.equalsIgnoreCase('gw2')) {
            ...
            if (action.equalsIgnoreCase('rank')) {
                ...
                if (args[1] && args[1].match(/^[1-9]+$/)) {
                    ...
                    if (UUIDs.indexOf(String(seasonID)) > -1) {
            ...
            /**
             * @commandpath gw2 local [language] - Set up the language of the API
             */
            if (action.equalsIgnoreCase('local')) {
                if (args[1]){
                    if (['en', 'fr', 'de', 'es', 'ko', 'zh'].indexOf(args[1].toLocaleLowerCase()) >= 0 ){
                        GW2_local = "&lang=" + args[1].toLocaleLowerCase();
                        $.say($.lang.get('guildwars2.local.set', args[1].toLocaleLowerCase()));
                    }
                    else {
                        $.say($.lang.get('guildwars2.local.man'));
                    }
                }
                else {
                    $.say($.lang.get('guildwars2.local.error'));
                }
            }
            ...
            if (action.equalsIgnoreCase('world') || action.equalsIgnoreCase('server')) {
                if (!GW2_world || GW2_world === undefined) {
                    data = JSON.parse(_getJSON(GW2_apiURL + '/v2/account?access_token=' + GW2_apiKey));
                    GW2_world = JSON.parse(_getJSON(GW2_apiURL + '/v2/worlds?ids=' + data.world.toString() + GW2_local));
            ...
            /**
             * @commandpath gw2 wvw - Commands linked to WvW.
             */
            if (action.equalsIgnoreCase('wvw')) {
                if (args[1]) {
                    if (args[1].toLocaleLowerCase() === "rank") {
                        if (!GW2_wvw_rank || GW2_wvw_rank === undefined) {
                            data = JSON.parse(_getJSON(GW2_apiURL + '/v2/account?access_token=' + GW2_apiKey));
                            GW2_wvw_rank = data.wvw_rank.toString();
                            GW2_commander = data.commander;
                            setTimeout(function () {
                                GW2_wvw_rank = undefined;
                            }, 1800000);
                        }
                        if (GW2_commander === true) {
                            $.say($.whisperPrefix(sender) + $.lang.get('guildwars2.wvw.rank.commander', username, GW2_wvw_rank));
                        } else {
                            $.say($.whisperPrefix(sender) + $.lang.get('guildwars2.wvw.rank', username, GW2_wvw_rank));
                        }
                        return;
                    }
                    else if (args[1].toLocaleLowerCase() === "matchup"){
                        if (!GW2_matchup_red_name.length || !GW2_matchup_blue_name.length || !GW2_matchup_green_name.length) {
                            data = JSON.parse(_getJSON(GW2_apiURL + '/v2/account?access_token=' + GW2_apiKey));
                            GW2_matchup = JSON.parse(_getJSON(GW2_apiURL + '/v2/wvw/matches?world=' + data.world.toString()));
                            GW2_matchup_red = JSON.parse(_getJSON(GW2_apiURL + '/v2/worlds?ids=' + GW2_matchup.all_worlds.red.toString() + GW2_local));
                            GW2_matchup_blue = JSON.parse(_getJSON(GW2_apiURL + '/v2/worlds?ids=' + GW2_matchup.all_worlds.blue.toString() + GW2_local));
                            GW2_matchup_green = JSON.parse(_getJSON(GW2_apiURL + '/v2/worlds?ids=' + GW2_matchup.all_worlds.green.toString() + GW2_local));

                            for (i = 0; i < GW2_matchup_red.length; i++){
                                GW2_matchup_red_name.push(GW2_matchup_red[i].name.toString());
                            }
                            for (i = 0; i < GW2_matchup_blue.length; i++){
                                GW2_matchup_blue_name.push(GW2_matchup_blue[i].name.toString());
                            }
                            for (i = 0; i < GW2_matchup_green.length; i++){
                                GW2_matchup_green_name.push(GW2_matchup_green[i].name.toString());
                            }
                            GW2_matchup_red_name = GW2_matchup_red_name.toString().replace(/,/g, ' + ');
                            GW2_matchup_blue_name = GW2_matchup_blue_name.toString().replace(/,/g, ' + ');
                            GW2_matchup_green_name = GW2_matchup_green_name.toString().replace(/,/g, ' + ');
                            setTimeout(function () {
                                GW2_matchup_red_name = [];
                                GW2_matchup_blue_name = [];
                                GW2_matchup_green_name = [];
                            }, 1800000);
                        }
                        $.say($.lang.get('guildwars2.wvw.matchup', GW2_matchup_red_name, GW2_matchup_blue_name, GW2_matchup_green_name));
                    }
                    else if (args[1].toLocaleLowerCase() === "victorypoints"){
                        if (GW2_wvw_vp === undefined) {
                            data = JSON.parse(_getJSON(GW2_apiURL + '/v2/account?access_token=' + GW2_apiKey));
                            GW2_matchup = JSON.parse(_getJSON(GW2_apiURL + '/v2/wvw/matches?world=' + data.world.toString()));
                            GW2_wvw_vp = GW2_matchup.victory_points;
                            setTimeout(function (){
                                GW2_wvw_vp = undefined;
                            }, 60000);
                        }
                        $.say("Red : " + GW2_wvw_vp.red + " Blue : " + GW2_wvw_vp.blue + " Green : " + GW2_wvw_vp.green);
                    }
                    else if (args[1].toLocaleLowerCase() === "score"){
                        if (GW2_wvw_score === undefined){
                            data = JSON.parse(_getJSON(GW2_apiURL + '/v2/account?access_token=' + GW2_apiKey));
                            GW2_matchup = JSON.parse(_getJSON(GW2_apiURL + '/v2/wvw/matches?world=' + data.world.toString()));
                            GW2_wvw_score = GW2_matchup.skirmishes[GW2_matchup.skirmishes.length -1].scores;
                            setTimeout(function (){
                                GW2_wvw_score = undefined;
                            }, 60000);
                        }
                        $.say("Red : " + GW2_wvw_score.red + " Blue : " + GW2_wvw_score.blue + " Green : " + GW2_wvw_score.green);
                    }
                    else if (args[1].toLocaleLowerCase() === "tick"){
                        if (GW2_red_tick === null || GW2_blue_tick === null || GW2_green_tick === null){
                            data = JSON.parse(_getJSON(GW2_apiURL + '/v2/account?access_token=' + GW2_apiKey));
                            GW2_matchup = JSON.parse(_getJSON(GW2_apiURL + '/v2/wvw/matches?world=' + data.world.toString()));
                            GW2_red_tick = 0;
                            GW2_blue_tick = 0;
                            GW2_green_tick = 0;
                            for (var i = 0; i < GW2_matchup.maps.length ; i++){
                                for (var j = 0 ; j < GW2_matchup.maps[i].objectives.length ; j++){
                                    switch (GW2_matchup.maps[i].objectives[j].owner){
                                        case 'Red':
                                            GW2_red_tick += parseInt(GW2_matchup.maps[i].objectives[j].points_tick);
                                            break;
                                        case 'Blue':
                                            GW2_blue_tick += parseInt(GW2_matchup.maps[i].objectives[j].points_tick);
                                            break;
                                        case 'Green':
                                            GW2_green_tick += parseInt(GW2_matchup.maps[i].objectives[j].points_tick);
                                            break;
                                    }
                                }
                            }
                        ...
                        $.say("Red : " + GW2_red_tick + " Blue : " + GW2_blue_tick + " Green : " + GW2_green_tick);
                    }
                    else {
                        $.say($.lang.get('guildwars2.wvw'));
                    }
                }
                else {
                    $.say($.lang.get('guildwars2.wvw'));
